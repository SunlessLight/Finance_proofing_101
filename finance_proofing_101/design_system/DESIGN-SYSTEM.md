...
> Department colors live in **two places**: the design system (tokens) and
> `getDepartmentColor()` in `script.js`. Adding a department means adding a branch there
> **and** a token here.
...
- ⚠️ **`.btn-blur` uses `text-black/60`** (60% black) — intentionally low-contrast because
  it sits over imagery, but it can fail contrast on busy/light photos. Prefer `.btn-ghost`
  or a solid button when legibility matters.
...
- **Focus:** interactive components carry a `focus-visible` ring
  ...
  replacement.
- **Disabled:** a disabled style (`opacity-50 cursor-not-allowed`) is defined in
  `tailwind-theme.css` for buttons — apply it to form submit buttons (resource newsletter,
  ALSTAR lookup, jacket order) while requests are in flight.
- **Contrast:** see the `.btn-blur` caution above. Test text-over-image combinations.
...
Documented, not yet applied (apply when the relevant feature is built):

- ⏳ **Disabled button state** — class exists; wire it into the form submit buttons.
- ⏳ **`.btn-blur` contrast** — left as-is (intentional over imagery); revisit if legibility
  issues appear.
